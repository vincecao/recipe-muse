...
export const DishHeroDetail = memo(function DishHeroDetail({ recipe }: { recipe: DbRecipe }) {
    ...
    <div className="absolute inset-x-0 bottom-10 p-3 sm:p-8">
      <div className="max-w-4xl mx-auto backdrop-blur-md rounded-xl sm:rounded-2xl p-3 sm:p-6 bg-white/20 border border-black/5 dark:bg-slate-800/70 dark:border-white/10">
            ...
            <span
              className={cn(
                'px-3 sm:px-4 py-1 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium whitespace-nowrap',
                {
                  ...
                  // Advanced
                  'bg-rose-100/80 text-rose-700 dark:bg-rose-900/30 dark:text-rose-300': rl.difficulty === 'Advanced',
                },
              )}
            >
              {rl.difficulty} Level
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-1.5 sm:gap-3 mb-3 sm:mb-4">
          {[
            { icon: <GoClock />, label: 'Prep Time', value: rl.time },
            { icon: <FaGripfire />, label: 'Calories', value: `${rl.calories} kcal` },
            { icon: <TiStarFullOutline />, label: 'Ingredients', value: `${rl.ingredientsCount} items` },
            { icon: <FaUsers />, label: 'Serving', value: rl.servingSize },
          ].map((metric, index) => (
            <div
              key={index}
        ...
        <div className="space-y-2 sm:space-y-4">
          ...
          {/* Allergens Section */}
          {rl.allergens && rl.allergens.length > 0 && (
            <div className="space-y-1 sm:space-y-2">
